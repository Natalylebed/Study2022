...
namespace Api.Services
{
    public class PostService
    {
        ...
        public async Task CreatePost(CreatePostRequest request)
        {
           ...
           var dbModel2=_mapper.Map<PostContent>(model);

            var comment = await _context.PostContents.FirstOrDefaultAsync(x => x.Post.Id == dbModel.Id);
            if (comment != null)
            {
                comment.Comment=model.Comments.LastOrDefault();                
